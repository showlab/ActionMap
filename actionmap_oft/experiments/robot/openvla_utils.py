...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    # Race-safe: serialize via flock on the config file, retry empty reads,
    # write atomically via tmp+rename. Multiple concurrent evals on the same
    # ckpt would otherwise race the read/write and one would see an empty file.
    import fcntl
    desired_auto_map = {
        "AutoConfig": "configuration_prismatic.OpenVLAConfig",
        "AutoModelForVision2Seq": "modeling_prismatic.OpenVLAForActionPrediction",
    }
    with open(config_path, "r+") as lock_f:
        ...
        for _ in range(5):
            ...
            raise RuntimeError(f"Could not read {config_path} after 5 retries")
        if config.get("auto_map") == desired_auto_map:
            print(f"auto_map already up-to-date at: {os.path.abspath(config_path)}")
            return
...
def _is_lora_only_checkpoint(checkpoint_path: str) -> bool:
    """Check if checkpoint has only LoRA adapter weights (no merged model).

    A LoRA-only checkpoint has a lora_adapter/ directory but no merged
    model-*.safetensors files.  These are produced when training with
    merge_lora_during_training=False.
    """
    has_merged = (
        os.path.exists(os.path.join(checkpoint_path, "model.safetensors.index.json"))
        or os.path.exists(os.path.join(checkpoint_path, "model.safetensors"))
    )
    has_adapter = os.path.isdir(os.path.join(checkpoint_path, "lora_adapter"))
    return not has_merged and has_adapter


def _get_lora_base_model_path(adapter_dir: str) -> str:
    """Resolve base model path from LoRA adapter config.

    Prefers the local cache path stored in adapter_config.json.  Falls back
    to the HF model ID ``openvla/openvla-7b`` when the local path does not
    exist (e.g. on a different machine).
    """
    config_path = os.path.join(adapter_dir, "adapter_config.json")
    with open(config_path, "r") as f:
        config = json.load(f)
    local_path = config.get("base_model_name_or_path", "")
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    Supports two checkpoint formats:
      1. **Merged checkpoint** (default): full model weights in
         model-*.safetensors.  Loaded directly via ``from_pretrained``.
      2. **LoRA-only checkpoint**: only ``lora_adapter/`` present (no merged
         safetensors).  Loads the base model, applies the LoRA adapter, and
         calls ``merge_and_unload()`` to produce the same weights.
    ...
    # --- LoRA-only checkpoint path -------------------------------------------
    if not model_is_on_hf_hub(checkpoint_path) and _is_lora_only_checkpoint(checkpoint_path):
        from peft import PeftModel

        adapter_dir = os.path.join(checkpoint_path, "lora_adapter")
        base_model_path = _get_lora_base_model_path(adapter_dir)
        print(f"Detected LoRA-only checkpoint. Base model: {base_model_path}")

        assert not getattr(cfg, "load_in_8bit", False) and not getattr(cfg, "load_in_4bit", False), (
            "Quantization (load_in_8bit / load_in_4bit) is not supported with LoRA-only checkpoints. "
            "Merge the checkpoint first using vla-scripts/merge_lora_weights_and_save.py."
        )
        ...
        # Load base model using locally registered classes (not Hub code).
        # trust_remote_code=False ensures from_pretrained uses the classes
        # registered above, avoiding version mismatches with Hub code.
        ...
        # Apply LoRA adapter and merge into base weights
        peft_vla = PeftModel.from_pretrained(base_vla, adapter_dir)
        vla = peft_vla.merge_and_unload()
        print("LoRA adapter merged successfully.")

    # --- Standard merged checkpoint path -------------------------------------
    else:
...
def get_processor(cfg: Any) -> AutoProcessor:
    ...
    For LoRA-only checkpoints the custom code files (processing_prismatic.py)
    may be absent, so we load the processor from the base model instead.  The
    processor (tokenizer + image processor) is identical between base and
    fine-tuned models.
    ...
    if not model_is_on_hf_hub(checkpoint_path) and _is_lora_only_checkpoint(checkpoint_path):
        adapter_dir = os.path.join(checkpoint_path, "lora_adapter")
        base_path = _get_lora_base_model_path(adapter_dir)
        return AutoProcessor.from_pretrained(base_path, trust_remote_code=True)
    return AutoProcessor.from_pretrained(checkpoint_path, trust_remote_code=True)
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead, HeatmapActionHead]:
    """
    ...
    Returns:
        Union[L1RegressionActionHead, DiffusionActionHead, HeatmapActionHead]: The initialized action head

    Raises:
        AssertionError: If more than one action head type is specified
    """
    use_heatmap = getattr(cfg, "use_heatmap", False)
    assert sum([cfg.use_l1_regression, cfg.use_diffusion, use_heatmap]) <= 1, (
    ...
    if cfg.use_l1_regression:
        ...
        action_head = HeatmapActionHead(
            ...
            decode_mode=getattr(cfg, "heatmap_decode_mode", "soft_argmax"),
            decode_temperature=getattr(cfg, "heatmap_decode_temperature", 1.0),
            decode_top_k=getattr(cfg, "heatmap_decode_top_k", 0),
            periodic_rot=getattr(cfg, "periodic_rot", False),
            grid_pad=getattr(cfg, "heatmap_grid_pad", 0.0),
        )
    else:
        raise ValueError("One of use_l1_regression, use_diffusion, or use_heatmap must be True")
...
def resize_image_for_policy(img: np.ndarray, resize_size: Union[int, Tuple[int, int]]) -> np.ndarray:
    ...
    if HAS_TF:
        ...
        # PIL fallback: JPEG round-trip + Lanczos resize (matches TF pipeline)
...
def crop_and_resize(image, crop_scale: float, batch_size: int):
    ...
    Args:
        image: TF Tensor (batch_size, H, W, C) or (H, W, C) with values in [0,1], or numpy array
        crop_scale: Area of center crop relative to original image
        batch_size: Batch size

    Returns:
        Cropped and resized image (TF Tensor or numpy array)
    """
    if HAS_TF:
        ...
        # PIL fallback for center crop + resize
        import math
        ...
        pil_img = Image.fromarray((cropped * 255).astype(np.uint8) if cropped.max() <= 1.0 else cropped)
...
def center_crop_image(image: Union[np.ndarray, Image.Image]) -> Image.Image:
    ...
    if HAS_TF:
        ...
        result = crop_and_resize(img_arr, crop_scale, batch_size)
