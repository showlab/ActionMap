...
class DeployConfig:
    ...
    heatmap_decode_mode: str = "soft_argmax"         # (When `use_heatmap==True`) Decoding: soft_argmax, hard_argmax, topk, mean
    heatmap_decode_temperature: float = 1.0          # (When `use_heatmap==True`) Temperature for softmax (T<1 sharpens)
    heatmap_decode_top_k: int = 0                    # (When `use_heatmap==True`) Top-k bins to keep (topk/mean modes only)
